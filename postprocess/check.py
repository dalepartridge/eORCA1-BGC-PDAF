...
def getMeanSTDCompute(year, month, it, varnames, basefilename):
    val = dict()
    for varname in varnames:
        val[varname] = dict()
        v = np.zeros((_Ne, _ny, _nx))
        for i in range(_Ne):
            with netCDF4.Dataset(f'{year}/{month}/ensemble_{i+1}/{varnames[varname]["file"]}', 'r') as f:
                v[i] = varnames[varname]['op'](*[f[vname][it, 0] for vname in varnames[varname]['args']] )
        val[varname]['mean'] = v.mean(axis=0)
        val[varname]['std'] = v.std(axis=0, ddof=1)
    return val


def getMeanSTDRead(year, month, it, varnames, basefilename):
    val = dict()
    for varname in varnames:
        val[varname] = dict()
        with netCDF4.Dataset(f'{year}/{month}/{varnames[varname]["file"]}', 'r') as f:
            val[varname]['mean'] = f[vname][it, 0]
            val[varname]['std'] = f[vname+'_std'][it, 0]
    return val
...
def getEnsMeanSTDCompute(varnames, year, cmonth, month, day):
    val = dict()
    for varname in varnames:
        val[varname] = np.zeros((_Ne, 2, _ny, _nx))
        for i in range(1, _Ne+1):
            f = netCDF4.Dataset(f'{year}/{cmonth}/analysis/state_chlorophyll_{year}{month}{day}_{str(i).zfill(3)}.nc', 'r')
...
def getEnsMeanSTDRead(varnames, year, cmonth, month, day):
    mean, std = dict(), dict()
    for varname in varnames:
        f = netCDF4.Dataset(f'{year}/{cmonth}/analysis/state_chlorophyll_{year}{month}{day}.nc', 'r')
        mean[varname] = f[varname][:]
        std[varname] = f[varname+'_std'][:]
        f.close()
    return mean, std
...
def plotDA(it, title, year, cmonth, month, obsvarnames):
    lons, lats = getCoord()
    mask = getMask()
    data, axProperties, figProperty = initialisePlot(n=6)

    for varname in obsvarnames:
        figProperty['title'] = title
        obs = getObservation(year, month, str(it+1).zfill(2))
        if month in ['04', '08', '12']:
            mean, std = getEnsMeanSTDCompute([varname], year, cmonth, month, str(it+1).zfill(2))
        else:
            mean, std = getEnsMeanSTDRead([varname], year, cmonth, month, str(it+1).zfill(2))
        mean, std = mean[varname], std[varname]
        ...
        data[2]['x'] = lons
        data[2]['y'] = lats
        data[2]['c'] = np.ma.masked_where(mask, std[0])
        axProperties[2]['cmap'] = 'cmo.amp'
        axProperties[2]['title'] = 'spread forecast'
        axProperties[2]['norm'] = mcolors.Normalize(vmax=vmax)

        data[3]['x'] = lons
        data[3]['y'] = lats
        data[3]['c'] = np.ma.masked_where(mask, std[1])
        axProperties[3]['cmap'] = 'cmo.amp'
        axProperties[3]['title'] = 'spread analysis'
        axProperties[3]['norm'] = mcolors.Normalize(vmax=vmax)

        data[4]['x'] = lons
        data[4]['y'] = lats
        data[4]['c'] = np.ma.masked_where(mask, std[0] - std[1])
        axProperties[4]['cmap'] = 'cmo.amp'
        axProperties[4]['title'] = 'spread reduction'
        vmaxR = np.quantile(data[4]['c'], 0.9)
        axProperties[4]['norm'] = mcolors.Normalize(vmax=vmaxR)

        data[5]['x'] = lons
        data[5]['y'] = lats
        data[5]['c'] = np.ma.masked_where(mask, r)
        axProperties[5]['norm'] =  mcolors.Normalize(vmax=vmax)
        axProperties[5]['cmap'] = 'cmo.amp'
        axProperties[5]['title'] = 'r'
...
def saveSpread(obsvarnames, year, cmonth, month, it):
    mask = getMask()
    if month in ['04', '08', '12']:
        _, std = getEnsMeanSTDCompute(obsvarnames, year, cmonth, month, str(it+1).zfill(2))
    else:
        _, std = getEnsMeanSTDRead(obsvarnames, year, cmonth, month, str(it+1).zfill(2))
    spread = dict()
    for varname in obsvarnames:
        spread[varname] = np.zeros(2)
        spread[varname][0] = std[varname][0][~mask].mean(axis=(-1))
        spread[varname][1] = std[varname][1][~mask].mean(axis=(-1))
...
def plotSpreadSpatialMean(year, months):
    ...
    t = []
    for month in months:
        rng = range(getMonthDays(year, month)) if month != '01' else range(15, getMonthDays(year, month))
        for it in rng:
...
def getObsSpace(var1, var2):
    var = var1 + var2
    var[var < 1e-14] = -14
    var[var >= 1e-14] = np.log10(var[var >= 1e-14])
    return var
...
if __name__ == '__main__':
    ...
    varnames =['CHD', 'CHN',]
    varnames =['CHD', 'CHN', 'PHD', 'PHD', 'PDS', 'toce_con', 'soce_abs']
    varnames = {'chlorophyll':{'op': getObsSpace, 'args': ['CHD', 'CHN', ], 'file':'eORCA1_1d_20150113_20150113_ptrc1_T_201501-201502.nc'},
                'nitrogen':{'op': getObsSpace, 'args': ['PHD', 'PHD', ], 'file':'eORCA1_1d_20150113_20150113_ptrc1_T_201501-201502.nc'},
                'PDS':{'op': lambda x: x, 'args': ['PDS', ], 'file':'eORCA1_1d_20150113_20150113_ptrc1_T_201501-201502.nc'},
                'toce_con':{'op': lambda x: x, 'args': ['toce_con', ], 'file':'eORCA1_1d_20150113_20150113_grid_T_201501-201502.nc'}, 
                'soce_abs':{'op': lambda x: x, 'args': ['soce_abs', ], 'file':'eORCA1_1d_20150113_20150113_grid_T_201501-201502.nc'}}
    year = '2015'
    month = '01'
    ilvl = 0
    ...
    # for cmonth in ['01', '03', '05', '07']:
    #     for m in range(2):
    #         month = str(int(cmonth) + m).zfill(2)
    #         rng = range(getMonthDays(year, month)) if month != '01' else range(15, getMonthDays(year, month))
            # processes = []
            # date = datetime.date(2015, int(month), rng.start + 1)
            # for i in rng:
            #     d = date + datetime.timedelta(days=i-rng.start)
            #     p = mp.Process(target=plotDA, args=(i, f'DAChlo_{d.strftime("%Y%m%d")}', '2015', cmonth, month, obsvarnames))
            #     p.start()
            #     processes.append(p)
            ...
            # for i in rng:
            #     d = date + datetime.timedelta(days=i-rng.start)
            #     p = mp.Process(target=saveSpread, args=(obsvarnames, '2015', cmonth, month, i))
            #     p.start()
            #     processes.append(p)
    ...
    plotSpreadSpatialMean('2015', [str(m).zfill(2) for m in range(1, 9)])