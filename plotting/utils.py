...
def get_land_mask() -> np.ma.MaskedArray:
...
def get_coord() -> tuple[np.ma.MaskedArray, np.ma.MaskedArray]:
    ...
    with netCDF4.Dataset(config.DOMAIN_PATH, 'r') as f:
        lons: np.ma.MaskedArray = wrap_data(f['nav_lon'][:])
        lats: np.ma.MaskedArray = f['nav_lat'][:]
    return lons, lats
...
def wrap_data(lons: np.ndarray) -> np.ndarray:
    ...
    -------
    np.ndarray:
        Array of wrapped longitude values.
    """
